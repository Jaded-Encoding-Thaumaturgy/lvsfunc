...
from vstools import CustomValueError, FormatsMismatchError, FrameRangesN, core, replace_ranges, vs
...
def custom_mask_clip(clip: vs.VideoNode, ref: vs.VideoNode | None = None,
    ...
    This is optimised from other implementations I've seen by splicing the masks into
    a blank clip, as opposed to opening up the image, masking, and then moving on to the next image.
    ...
    if ref and clip.format != ref.format:
        raise FormatsMismatchError(custom_mask_clip)

    if len(imgs) != len(ranges):
        raise CustomValueError("`imgs` and `ranges` must be of the same length!", custom_mask_clip,
                               reason=f"{len(imgs)} != {len(ranges)}")
