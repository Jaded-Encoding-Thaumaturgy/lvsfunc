...
from vstools import Keyframes, MismatchRefError, find_prop_rfs, merge_clip_props, vs
...
def dynamic_scene_adaptive_grain(
    ...
    This function may help reduce overall filesize by applying a weaker grained clip on brighter
    scenes, which should hopefully reduce the amount of graining applied on the clip overall.
    As such, it's recommended you use dynamic grain on darker scenes,
    and static on brighter ones with this function.
    ...
    Args:
        clip: The clip used for metric gathering and, if necessary, keyframe generation.
        grain_dark: Grained clip for dark scenes.
        grain_bright: Grained clip for bright scenes.
        keyframes: Keyframes used to sample scene averages.
            Generated from ``clip`` when ``None``.
            Default: ``None``.
        thr: Luminance threshold for choosing between ``grain_dark`` and ``grain_bright``.
            Lower values favor ``grain_bright``.
            Default: ``0.55``.
        key: Property key written by :class:`~vstools.functions.timecodes.SceneAverageStats`.
            Capitalized automatically.
            Default: ``"Grain"``.

    Returns:
        Clip with different types of graining applied based on the scene's luminosity.
    """
