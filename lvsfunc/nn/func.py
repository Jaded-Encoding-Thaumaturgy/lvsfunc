...
def clip_to_npy(
    ...
    try:
        ...
        pbar = tqdm(
            total=total_frames,
            unit="frame",
            desc=f"Dumping numpy arrays to {out_dir}/...",
        )
    except ImportError:
        pbar = None
...
def npy_to_clip(
    ...
    if first_frame.ndim < 2:
        raise NumpyArrayLoadError(
            f"Invalid frame shape: {first_frame.shape}. Expected at least 2 dimensions.",
            func,
        )
    ...
    def _read_frame(n: int, f: vs.VideoFrame) -> vs.VideoFrame:
        ...
        if loaded_frame.ndim < 2:
            raise NumpyArrayLoadError(
                f"Invalid frame shape at index {n}: {loaded_frame.shape}. Expected at least 2 dimensions.",
                func,
            )
        ...
        if loaded_frame.ndim == 2:
            ...
            if loaded_frame.shape[0] == 3:
                ...
                raise NumpyArrayLoadError(
                    f"Unexpected frame shape: {loaded_frame.shape}. Expected (channels, height, width)",
                    func,
                )
        else:
            raise NumpyArrayLoadError(
                f"Invalid frame dimensions: {loaded_frame.ndim}. Expected 2 (GRAY) or 3 (YUV)",
                func,
            )
